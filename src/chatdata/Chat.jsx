...
function Chat() {
  ...
  let userdataa = JSON.parse(localStorage.getItem("userid"));

  let currentuser = sessionStorage.getItem("currentuser");
  let allmaindata = JSON.parse(localStorage.getItem("alldata"));
  ...
  let filteredArray = userdata.filter((item) => item.username === search);
  useEffect(() => {
    allmaindata?.map((item) => {
      console.log(currentuserdata?.id, item.id);
      if (item?.id === currentuser) {
  ...
  useEffect(() => {
    ...
    socket.on("usermessagedekho", (userdataa) => {
      if (chatContainerRef.current) {
        chatContainerRef.current.scrollTop =
          chatContainerRef.current.scrollHeight;
      }
    ...
    socket.on("disusermessagedekho", (userdatadis) => {
      let filteredArray = JSON.parse(localStorage.getItem("userid")).filter(
        (item) => item.id !== String(userdatadis.id)
      );

      localStorage.setItem("userid", JSON.stringify(filteredArray));
    ...
    <div class="b2">
      ...
      setOnline({ id: userdatadis.id, online: false });

      diss.play();
    });

    socket.on("message", (userdataa) => {
      if (chatContainerRef.current) {
        chatContainerRef.current.scrollTop =
          chatContainerRef.current.scrollHeight;
      }
  ...
  function send() {
    ...
    if (dataall.length === 0) {
      ...
      localStorage.setItem("alldata", JSON.stringify(dataall));
    } else {
      dataall.push({
        message: message,
        username: currentuserdata?.username,
